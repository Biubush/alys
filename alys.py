...
import threading,os,sys,random,smtplib,time,qrcode,logging
...
def startAligo(owner:str,email:str=None)->Aligo:#输入用户名和邮件(可空)，登录阿里云并返回生成的阿里云对象
    ...
    if not email:
        ...
        ALIGOS[owner]=Aligo(name= owner,email=(email, '请重新登陆'),level=logging.INFO)
...
def writeDialog(owner:str,content:str):#输入用户名和日志内容，对指定用户进行日志的增操作
    ...
    if countLines(dialog)>=100:
        ...
        for line in last_100_lines:
            dialog=dialog+'\n'+line
    User.query.filter(User.username==owner).first().dialog=str(dialog)+'\n['+nowTime()+']'+content
    db.session.commit()
...
def sendHtml(mailAdress:str,receieverName:str,Title:str,wholeText:str):#输入收件人邮箱，收件人昵称，邮件标题，邮件正文，发送指定邮件
    global ADMIN
                ...
                <div class="container p-4 my-4 border">
                    <div class="row">
                        ...
                        <div class="col-md-6 container p-4 my-4 border">
                            ...
                            <hr>
                            <p>尊敬的'''+receieverName+''':</p>
                            <p class="indented">您好!</p>
                            <p class="indented">'''+wholeText+'''</p>
                            <br>
                            <br>
                            <div class="text-end">
        ...
        if not os.path.exists("./alys.db"):
            ...
            for user in User.query.all():
                if not user.baned:
                    ...
                    ban_count+=1
            writeAdminDialog('登录了'+str(aligo_count)+'个用户的阿里云盘,有'+str(ban_count)+'个用户被禁而未登录')
            enable_count=0
...
def getLog(username:str,lines:int):#获取某人日志的后多少行
    dialog=getLastLines(str(User.query.filter(User.username==username).first().dialog),lines)
    text=''
    for line in dialog:
        text=text+'\n'+line
    return text
...
def getMidStr(s:str, start_str:str, stop_str:str)->str:#输入欲取文本，前置文本，后置文本，取出其中间的文本并返回
...
def saveFolder(owner:str,share_id:str,from_id:str,to_id:str,key:str=None)->int:#转存文件
    passcount=0
    save_name_list=[]
    if not key:
    ...
    filelist=ALIGOS[owner].get_share_file_list(share_token=share_token,parent_file_id=from_id)
    from_name_list=[]
    from_id_dict={}
    ...
    for name in save_name_list:
        ALIGOS[owner].share_file_saveto_drive(file_id=from_id_dict[name],share_token=share_token,to_parent_file_id=to_id)
    if len(filelist)-passcount != 0:
        writeDialog(owner=owner,content='文件保存成功,跳过'+str(passcount)+'个已存在文件,保存了'+str(len(filelist)-passcount)+'个文件')
    ...
    with app.app_context():
        while Task.query.filter_by(name=taskname,owner=owner).first().switch:
            ...
            try:
                ...
                if saveFolder(owner=owner,share_id=task.share_id,from_id=task.from_id,to_id=task.to_id,key=task.key)==0:
                    if not flag:
                        writeDialog(owner=owner,content='任务【'+taskname+'】进入循环等待...')
                        flag=True
...
def online():
    ...
    if not g.user:
        ...
        if request.method=='GET':
            ...
            return render_template('signin.html',src='../static/img/qrcode/'+USER+'.png')
        else:
            globals
...
def usercenter():
    if not g.user:
        ...
        if 'admin' in session:
            ...
            for user in users:
                if search_task:
                    tasklist[user.username]=Task.query.filter(Task.name.contains(search_task),Task.owner==user.username).all()
                else:
                    tasklist[user.username]=Task.query.filter_by(owner=user.username).all()
            members=len(users)
            ...
            if g.user.baned!=1:
                ...
                if not search:
                    ...
                    total=len(tasks)
                    enabled=len(Task.query.filter_by(owner=g.user.username,switch=1).all())
                else:
                    tasks=Task.query.filter(Task.name.contains(search),Task.owner==g.user.username).all()
                    total=len(tasks)
                    enabled=len(Task.query.filter(Task.name.contains(search),Task.owner==g.user.username,Task.switch==1).all())
                dialog=''
                for line in getLastLines(g.user.dialog,5):
                    dialog=dialog+line+'\n'
                return render_template('usercenter.html',userdata=g.user,tasks=tasks,total=total,enabled=enabled,dialog=dialog)#个人中心页面
            else:
    ...
    if not g.user:
        ...
        if request.method=='POST':
            try:
                ...
                with app.app_context():
                    db.session.commit()
                if task.switch==1 and scheduler.get_job(job_id=old_name+'_'+owner):
    ...
    if 'admin' in session:
        try:
            ...
            if os.path.exists(str(Path.home().joinpath('.aligo'))+'/'+user.username+'.json'):
                os.remove(str(Path.home().joinpath('.aligo'))+'/'+user.username+'.json')
...
def createfolder():
    if not g.user:
        ...
        try:
            ...
            ALIGOS[g.user.username].create_folder(name=foldername,parent_file_id=parent_id)
    ...
    if not g.user:
        ...
        try:
            ...
            if to_id=='' or to_id=='None':
                return '选择文件夹非法'
            savecount=saveFolder(g.user.username,share_id=share_id,from_id=parent_file_id,to_id=to_id,key=request.form.get('key'))
            if savecount==0:
                writeDialog(owner=g.user.username,content='经过本次测试，无需要更新的文件')
            else:
                writeDialog(owner=g.user.username,content='经过本次测试，更新了'+str(savecount)+'个文件')
